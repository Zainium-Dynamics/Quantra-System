...
///
/// Install: `/usr/bin/quantra-ctl` — no symlinks required.
...
enum ControlCommand {
    ...
    Assay { service: String },
    Tree,
    List,
    Metrics,
    Isolate { service: String, exit_isolation: bool },
    Shutdown { reboot: bool },
    Signal { service: String, signal: String },
    IsStarted { service: String },
    IsFailed { service: String },
    Setenv { name: String, value: Option<String> },
    AddDep { from: String, to: String, dep_type: String },
    RmDep { from: String, to: String },
}
...
fn build_command(cmd: &Cmd) -> ControlCommand {
    match cmd {
        Cmd::Start { service } => ControlCommand::Start { service: service.clone() },
        Cmd::Stop { service } => ControlCommand::Stop { service: service.clone() },
        Cmd::Restart { service } => ControlCommand::Restart { service: service.clone() },
        Cmd::Reload { service } => ControlCommand::Reload { service: service.clone() },
        Cmd::Kill { service } => ControlCommand::Kill { service: service.clone() },
        Cmd::Enable { service } => ControlCommand::Enable { service: service.clone() },
        Cmd::Disable { service } => ControlCommand::Disable { service: service.clone() },
        Cmd::Status { service } => ControlCommand::Status { service: service.clone() },
        Cmd::Assay { service } => ControlCommand::Assay { service: service.clone() },
        ...
        Cmd::IsStarted { service } => ControlCommand::IsStarted { service: service.clone() },
        Cmd::IsFailed  { service } => ControlCommand::IsFailed  { service: service.clone() },
        Cmd::Setenv { assignment } => {
            if let Some((name, value)) = assignment.split_once('=') {
                ControlCommand::Setenv { name: name.to_string(), value: Some(value.to_string()) }
            } else {
                ControlCommand::Setenv { name: assignment.clone(), value: None }
            }
        }
...
fn send_command(socket_path: &str, cmd: &ControlCommand) -> Result<CtlResponse> {
    ...
    stream.write_all(&len).context("Failed to send command length")?;
    stream.write_all(&payload).context("Failed to send command payload")?;
    stream.flush().ok();
    ...
    stream.read_exact(&mut resp_buf).context("Failed to read response")?;
...
fn render_response(resp: &CtlResponse, cmd: &Cmd) {
    let icon = if resp.ok { "\x1b[32m✓\x1b[0m" } else { "\x1b[31m✗\x1b[0m" };
...
fn render_status(resp: &CtlResponse) {
    ...
    let state_color = if state == "running" { "\x1b[32m" } else { "\x1b[90m" };

    println!("\n\x1b[1m● {}\x1b[0m   {}{}\x1b[0m", name, state_color, state);
    ...
    println!("  \x1b[38;5;39mPID\x1b[0m        {}", if pid > 0 { pid.to_string() } else { "—".into() });
    println!("  \x1b[38;5;39mUptime\x1b[0m     {}", format_duration(uptime));
    ...
    println!("  \x1b[38;5;39mEnabled\x1b[0m    {}", if enabled { "\x1b[32myes\x1b[0m" } else { "\x1b[90mno\x1b[0m" });

    if let Some(log_lines) = data["log_tail"].as_array() {
        if !log_lines.is_empty() {
            println!("\n  \x1b[38;5;244m── Last {} log lines ──────────────────────\x1b[0m", log_lines.len());
            for line in log_lines {
                let l = line.as_str().unwrap_or("");
                let color = if l.contains("ERROR") { "\x1b[31m" }
                            else if l.contains("WARN") { "\x1b[33m" }
                            else { "\x1b[38;5;244m" };
...
fn render_list(resp: &CtlResponse) {
    ...
    println!("\n  \x1b[1m{:<24} {:<10} {}\x1b[0m", "SERVICE", "STATE", "PID");
    ...
    if let Some(data) = &resp.data {
        if let Some(services) = data["services"].as_array() {
            for svc in services {
                ...
                let state_str = if running { "\x1b[32mrunning\x1b[0m" } else { "\x1b[90mstopped\x1b[0m" };
                let pid_str = if pid > 0 { pid.to_string() } else { "—".into() };
