...
fn main() {
    ...
    let req: serde_json::Value = match cmd {
        ...
        "set-linger"        => {
            let uid: u32 = arg(&args, 2, "uid");
            let enable = args.get(3).map(|s| s == "yes" || s == "1" || s == "true").unwrap_or(false);
        ...
        "get-brightness"    => {
            let subsystem = args.get(2).cloned().unwrap_or_else(|| "backlight".to_string());
            let name      = args.get(3).cloned().unwrap_or_else(|| usage("get-brightness <subsystem> <name>"));
            serde_json::json!({"cmd": "get_brightness", "subsystem": subsystem, "name": name})
        }
        "set-brightness"    => {
            let subsystem = args.get(2).cloned().unwrap_or_else(|| "backlight".to_string());
            let name      = args.get(3).cloned().unwrap_or_else(|| usage("set-brightness <subsystem> <name> <value>"));
...
fn call(req: serde_json::Value) -> Result<serde_json::Value, String> {
    ...
    stream.read_exact(&mut resp_buf).map_err(|e| format!("read resp: {}", e))?;
